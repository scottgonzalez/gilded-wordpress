...
Client.prototype.parsePost = function( postPath, callback ) {
	callback = callback.bind( this );

	fs.readFile( postPath, { encoding: "utf8" }, function( error, content ) {
		if ( error ) {
			return callback( error );
		}
		...
		if ( content.substring( 0, 8 ) === "<script>" ) {
			try {
				...
				return callback( new Error( "Invalid JSON metadata for " + postPath ) );
			}
		}
...
Client.prototype.validatePosts = function( callback ) {
	callback = callback.bind( this );

	var count = 0;
	var postPaths = {};

	this.walkPosts( this.path( "posts/" ), function( post, callback ) {
...
Client.prototype.syncPosts = function( termMap, callback ) {
	callback = callback.bind( this );
