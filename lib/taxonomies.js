...
Client.prototype.validateTerms = function( callback ) {
	callback = callback.bind( this );

	var count = 0;

	this.readTerms( this.path( "taxonomies.json" ), function( error, taxonomies ) {
		if ( error ) {
			return callback( error );
		}
...
Client.prototype.syncTerms = function( callback ) {
	callback = callback.bind( this );
